    ...
    def init_NN(self, R, Faults, lr, Layers):
        ...
        with tf.name_scope("FLearners"):
            ...
            for i in range(1,len(Layers)):
                ...
                Layers[i], act=tf.nn.sigmoid, trainability = False, key = 'FL_layer'+str(i))
        ...
        with tf.name_scope("Trainer"):
            ...
            with tf.name_scope('Evaluation'):
                ...
                with tf.name_scope('Prob'):
                    self.Trainer['prob'] = tf.cast( tf.nn.softmax(self.classifier['Fault']), tf.float32 )
...
def PreProcess(num, dimension, flag):
    T, Y = DataImport(num=num, sample_size = 5000, features = 1000)
    if flag ==0:
...
def Analyse(TrainX, TestX, TrainY, TestY):
    ...
    model = Agent()
    model = model.init_NN(TrainX, 4, 0.1, [TrainX.shape[1], 20, 20])
    ####################################################################################
    ## Start the learning Procedure Now
    acc = 0.0
    # Declare a saver
    try:
        t = tqdm(xrange(Train_Glob_Iterations))
        for i in t:
            ...
            if i % 1 == 0:
                summary, a  = model.sess.run( [model.Summaries['merged'], model.Trainer['accuracy']], feed_dict={ model.Deep['FL_layer0'] : TestX, model.classifier['Target'] : TestY})
                print "Accuracy", a
            if a > 0.99:
                summary, pr  = model.sess.run( [ model.Summaries['merged'], model.Trainer['prob'] ], \
                feed_dict ={ model.Deep['FL_layer0'] : TestX, model.classifier['Target'] : TestY } )
                break
...
for x in tqdm(xrange(1)):
    ...
    TrainX, TestX, TrainY, TestY = train_test_split(P, Y, test_size = 0.60)
    acc = Analyse(TrainX, TestX, TrainY, TestY)
np.savetxt('Prob.csv', acc, delimiter=',')
