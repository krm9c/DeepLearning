...
print ("max_labels", max(valid_labels))
...
# The script has started working...
...
class Weights:
    def __init__(self, batch_size, num_input, num_output, num_final,
                 ...
                 weight_uni_range = 1, back_uni_range = 0.4):
...
def program():
    try:
        ...
        with tf.Session(graph=graph) as session:
            ...
            a = accuracy(test_prediction.eval(), test_labels)
            print("Previous Accuracy", a, "Initialized for the new one")
...
import gc
for gstep in tqdm(range(Glob_iteration)):
    Test.append(program())

Test = np.array(Test)
print (len(Test))
print("Mean", Test.mean(), "max", max(Test), "min", min(Test))
